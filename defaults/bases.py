    ...
    def get_trans_list_albumentations(self, transform_dict):
        ...
        if "Normalize" in transform_dict:            
            if transform_dict['Normalize']:
                ...
                print("Max pixel values: ", self.max_pixel_value if self.max_pixel_value is not None else 255.0)
                
                transform_list.append(A.Normalize(mean=self.mean, 
                                                std=self.std,
                                                max_pixel_value= self.max_pixel_value if self.max_pixel_value is not None else 255.0))         
...
class BaseModel(nn.Module):
    ...
    def BN_to_OtherNorm(self, normtype, submodel=None, num_groups=32, keep_stats=False):
        ...
        def recur_depth(normtype, submodel,lname, n=0, keep_stats=True, num_groups=32):
            if n < len(lname)-1:
                return recur_depth(normtype, get_atr(submodel,lname[n]),
                ...
                if normtype == "GroupNorm":
                    ...
                    raise NotImplementedError(f"BatchNorm cannot be replaced with {normtype}")
                if keep_stats:
        ...
        for name, module in submodel.named_modules():
            if isinstance(module,  nn.BatchNorm2d):
                recur_depth(normtype, submodel,name.split('.'), 
                            keep_stats=keep_stats, num_groups=num_groups)             
        ...
        if os.path.isfile(self.model_path) and self.restore_session:        
            ...
            if is_parallel(self.model):
                if hasattr(self.model.module, "teacher_encoder"):
                    self.model.module.teacher_encoder.load_state_dict(checkpoint['state_dict'])
                elif hasattr(self.model.module, "online_encoder"):
                    self.model.module.online_encoder.load_state_dict(checkpoint['state_dict'])                    
                else:
                    self.model.module.load_state_dict(checkpoint['state_dict'])
            else:
                if hasattr(self.model, "teacher_encoder"):
                    self.model.teacher_encoder.load_state_dict(checkpoint['state_dict'])
                elif hasattr(self.model, "online_encoder"):
                    self.model.online_encoder.load_state_dict(checkpoint['state_dict'])                                        
                else:
                    ...
                    try:
                        ...
                        dimension_miss_match_state_dict = self.fix_dims_loading_session(checkpoint['state_dict'])
                        
                        
                        dif_keys = self.model.load_state_dict(dimension_miss_match_state_dict, strict=False)
                        dif_keys = set([" : ".join(key.split(".")[:2]) for key in dif_keys.unexpected_keys])                     
                        if (dif_keys != {'fc : mlp', 'fc : last_layer'}) and (len(dif_keys) != 0):
                            raise RuntimeError(f"Error(s) in loading state_dict for {self.model.__class__.__name__}"
                                               f"unexpected_keys: {dif_keys}")
                        print("\033[93mIncompatibleKeys found for fc (assuming BYOL or DINO is models are used)\033[0m")
