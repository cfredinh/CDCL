...
class DefaultWrapper:
    ...
    def instantiate(self):        
        ...
        num_domains   = self.dataloaders.trainloader.dataset.num_domains
        examples_in_each_domain = self.dataloaders.trainloader.dataset.examples_in_each_domain
        ...
        self.model_params.num_domains = num_domains
        self.model_params.examples_in_each_domain = examples_in_each_domain
        ...
        optimizer_defs = self.init_optimizer(self.model, self.optimization_params.default, 
                                             aux_params=self.optimization_params.aux_head 
                                             if "aux_head" in self.optimization_params else None)  
        ...
        self.aux_schedulers = None        
        if "aux_head" in self.optimization_params:
            aux_scheduler_defs = self.init_scheduler(self.aux_optimizer,
                                                  self.optimization_params.aux_head, 
                                                  len(self.dataloaders.trainloader), 
                                                  epochs)   
            self.aux_schedulers = MixedLRScheduler(**aux_scheduler_defs)        
    ...
    def init_dataloaders(self, collate_fn=None) -> edict:
        ...
        if self.training_params.knn_eval_every or not self.is_supervised:
            knn_basesize = self.training_params.restrict_knn_basesize
            feature_bank_set = DataSet(self.dataset_params, mode='train', fb=True, use_subset=knn_basesize)
    ...
    @staticmethod
    def init_optimizer(model, optimization_params:edict, aux_params=None) -> edict:    
        ...
        if aux_params is not None:
            aux_optimizer_type = aux_params.optimizer.type
            aux_opt = optim.__dict__[aux_optimizer_type]
            aux_opt_params = aux_params.optimizer.params            
        included     = [n for n,_ in model.named_parameters() if 'aux_fc' not in n]
        included_aux = [n for n,_ in model.named_parameters() if 'aux_fc' in n]
        
        optimizer = opt(DefaultWrapper.get_params_groups(model, included), **opt_params)
        if aux_params is not None:
            aux_optimizer = aux_opt(DefaultWrapper.get_params_groups(model, included_aux), **aux_opt_params)
        else:
            aux_optimizer_type = optimizer_type
            aux_optimizer = opt(DefaultWrapper.get_params_groups(model, included_aux), **opt_params)
        # handling LARS
        ...
        return edict({"optimizer":optimizer, "optimizer_type":optimizer_type, 
                      "aux_optimizer":aux_optimizer, "aux_optimizer_type":aux_optimizer_type})
    
    @staticmethod
    def get_params_groups(model, included=[]):
        """
    ...
    def update_augmentation_strategy(self, parameters):
        ...
        if "augmentation_pipeline" in  parameters["dataset_params"]:
            print("Setting input json")
            augmentation_path = parameters["dataset_params"]["augmentation_pipeline"]
        else:
            augmentation_path = "augmentation_strategy-RxRx1.json"
            print("Getting augmentation pipeline")

        new_strategy_dir = os.path.join(self_dir, augmentation_path)   
    ...
    def dataset_mapper(self):
        return {
            "RxRx1_HUVEC": RxRx1_HUVEC,
            "CPG0004_full_img_large": CPG0004_full_img_large
            }
