...
class Attention(nn.Module):
    ...
    def forward(self, x, use_norm_attn=False):
        B, N, C = x.shape
        ...
        if use_norm_attn:
            norm_attn = (q @ k.transpose(-2, -1)) / (q.abs() @ k.transpose(-2, -1).abs())
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False, return_intermediate=False, use_norm_attn = False):
        if use_norm_attn:
            y, attn, norm_attn = self.attn(self.norm1(x), use_norm_attn)
        else:
...
class VisionTransformer(nn.Module):
    ...
    def forward_get_last_attention(self, x):
        ...
        if self.is_memory_efficient:
            for start_idx, _x in enumerate(x):
                _out, _, values = self.forward_features_get_last_attention(_x)
                ...
                if start_idx == 0:
                    ...
                    output = torch.cat((output, _out))
                    attn_output = torch.cat((attn_output, attn_))
            return output, attn_output
                
        else:     
            print("NOT MEMORY EFFICIENT")
            idx_crops = torch.cumsum(torch.unique_consecutive(
                torch.tensor([inp.shape[-1] for inp in x]),
                return_counts=True,
            )[1], 0)
            start_idx = 0
            for end_idx in idx_crops:
                _out, attn_ = self.forward_features_get_last_attention(torch.cat(x[start_idx: end_idx]))
                if start_idx == 0:
                    output = _out
                    attn_output = attn_
                else:
                ...
                start_idx = end_idx
            # Run the head forward on the concatenated features.
            return self.fc(output), attn_output
    
    def forward_features_get_last_attention(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
        ...
        x, attn = self.blocks[-1](x, return_attention=True, return_intermediate=True)
    ...
    def forward_features_get_attention(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
...
def deit_huge(patch_size=14, img_size=[224], pretrained=False, **kwargs):
    model = VisionTransformer(img_size=img_size,
        patch_size=patch_size, embed_dim=1280, depth=32, num_heads=16, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    if pretrained:
        checkpoint = download_weights("deit_huge", patch_size, kwargs['pretrained_type'])
        model.load_state_dict(checkpoint)    
    return model
