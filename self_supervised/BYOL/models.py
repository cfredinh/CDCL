...
class BARLOW_loss(nn.Module):
    def __init__(self, barlow_loss_weight=0.75, out_dim=2048, barlow_lambda_off_diag=0.95):
        super().__init__()
        ...
        self.barlow_scaling_factor = 1.0*(1.0-self.barlow_lambda_off_diag) + 0.01*self.barlow_lambda_off_diag
    ...
    def forward(self, x, y):
        ...
        c  = self.bn(x[0]).T @ self.bn(y[0])
        c += self.bn(x[1]).T @ self.bn(y[1])

        c.div_(batch_size*2.0) # Normalize by batch size, multiply by two as samples are used twice by deatatching on sample each time
        #c.div_(batch_size)
        if ddp_is_on():
            dist.all_reduce(c)
            synchronize()
        ...
        return (on_diag*(1.0-self.barlow_lambda_off_diag) + self.barlow_lambda_off_diag * off_diag)/self.barlow_scaling_factor
...
class BYOL(BaseModel):
    def __init__(
        ...
        if use_momentum:
            ...
            if use_barlow:
                self.loss_fn = BARLOW_loss(barlow_loss_weight=0.75, out_dim=prediction_hidden_size)
        else:
    ...
    def forward(self, x, return_embedding = False, adverserial_targets = None):
        with autocast(self.use_mixed_precision):
            if return_embedding:
                ...
                online_out = self.online_encoder.backbone(x)
                if self.online_encoder.aux_fc is not None:
                    aux_outs = self.online_encoder.aux_fc(online_out)
                return None, online_out, aux_outs

            x = [x_view.to(self.device_id, non_blocking=True) for x_view in x]

            # get the 2 augmented views of the online net
            x_online, x_emb, aux_out = zip(*[self.online_encoder(x_view, return_embedding=True) for x_view in x])
            ...
            with torch.no_grad():
                ...
                x_target = [target_encoder(x_view)[0] for x_view in x][::-1] # zero indexing for the selection of the output head and not the aux head 

            return self.loss_fn(x_online, x_target), aux_out[0]
